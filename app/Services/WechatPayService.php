...
class WechatPayService
{
     ...
     * 创建Native支付订单（扫码支付）
     */
    public function createNativeOrder(string $orderNo, string $description, float $amount, string $clientIp = ''): array
    {
        $params = [
            'appid' => $this->appId,
            'mch_id' => $this->mchId,
        ...
        if (!$result) {
            return ['success' => false, 'message' => '微信支付接口返回数据解析失败'];
        }
        
        if ($result['return_code'] !== 'SUCCESS') {
            return ['success' => false, 'message' => $result['return_msg'] ?? '微信支付接口调用失败'];
        }
        ...
        if (!$this->verifySign($result)) {
            return ['success' => false, 'message' => '微信支付返回签名验证失败'];
        }
